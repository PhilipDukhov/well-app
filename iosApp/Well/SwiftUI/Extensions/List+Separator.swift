...
private struct ListSeparatorModifier: ViewModifier {
    ...
    private func dividerSeeker() -> some View {
        DividerLineSeekerView { divider in
            //If we encounter a separator view in this hierarchy hide it
            switch self.style {
            case .none:
                if let superview = divider.superview {
                    if divider.frame.minY < 0 {
                        divider.isHidden = true
                    } else if divider.frame.origin.x > 0 {
                        divider.frame.size.width = superview.bounds.width
                        divider.frame.origin.x = 0
                    }
                }
                print(divider, divider.superview != nil)
            case .singleLine:
        ...
        //Set frame to +1 of max divider height that way we don't also attempt to change this view
...
final class InjectView: UIView {
    ...
    private func findScrollView(in view: UIView) -> UIScrollView? {
        ...
        //Found the scrollview so just return it
...
final class DividerHandlingView: UIView {
    var divider: (UIView) -> Void
    var table: (UITableView) -> Void
