...
fun String.toUpperSnakeCase(): String {
    ...
    // lower -> LOWER
    if (this.all { it.isLowerCase() }) return this.toUpperCase(Locale.getDefault())
...
fun String.toLowerCamelCase(): String {
    ...
    // lower -> LOWER
    if (this.all { it.isUpperCase() }) return this.toLowerCase(Locale.getDefault())
    // lowerCamel -> LOWER_CAMEL
...
fun String.toPascalCase(): String = when {
    all { it.isUpperCase() } -> {
        ...
        this[0] + substring(1).toLowerCase(Locale.getDefault())
    }
    all { it.isLowerCase() } -> {
